...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (searchBtn && searchForm && cancelBtn && searchInput) {
        ...
        // Add to Softly Label's Cart (Updated)
        ...
        document.querySelectorAll('.add-to-cart').forEach(button => {
            button.addEventListener('click', () => {
                ...
                //Update to handle stock limits (NEW)
        ...
        function updateCartDisplay() {
            ...
            cart.forEach((item, index) => {
                ...
                itemDiv.innerHTML = `
                    ...
                    <div class="item-details">
                        <h3>${item.name}</h3>
                        <p>${item.price.toFixed(2)} BND x ${item.quantity || 1}</p>
                    </div>
            ...
            document.querySelectorAll('.remove-btn').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    cart.splice(index, 1);
                    showPopupMessage(`${cart[index]?.name || "Item"} has been removed from your cart.`);
                    localStorage.setItem('cart', JSON.stringify(cart));
                    updateCartDisplay();
    ...
    function updateCheckoutSummary() {
        ...
        if (cart.length === 0) {
            ...
            cart.forEach(item => {
                ...
                itemDiv.innerHTML = `
                    <p class="item-name">${item.name} x ${item.quantity || 1}</p>
                    <p class="item-price">${itemTotal.toFixed(2)} BND</p>
                `;
        ...
        if (subtotal >= 60) {
            ...
            shippingOptions.forEach(option => {
                if (option.checked) {
                    selectedDeliveryArea = option.value;
                }
            });
    ...
    shippingOptions.forEach(option => {
        ...
        //Popup message when purchasing (NEW)
        function handlePurchase(e) {
        e.preventDefault(); 

        const cartNow = JSON.parse(localStorage.getItem('cart') || '[]');
        if(!cartNow || cartNow.length === 0) {
            showPopupMessage("Your cart is empty. Please add items before purchasing.");
            return;
        }
